...
public class MybatisConfiguration {
    ...
    public MybatisSqlSessionFactoryBean sqlSessionFactoryBean(@Qualifier(value = "dataSource") DataSource dataSource) throws Exception {
        ...
        try {
            ...
            result.addAll(Arrays.asList(resolver.getResources("classpath*:com.sqt.edu.admin.config/mapper/*/*.xml")));
            result.addAll(Arrays.asList(resolver.getResources("classpath*:mapper/*/*.xml")));
        } catch (IOException e) {
            log.error("获取【classpath:mapper/*/*.xml,classpath:com.sqt.edu.admin.config/mapper/*/*.xml】资源错误!异常信息:{}", e);
        }
